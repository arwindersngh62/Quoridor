...
def get_action_from_string(string):
    ...
    if len(string) == 2:
        ...
        if (
            string[1] in [str(i) for i in range(9)]
            and string[2] in [str(i) for i in range(9)]
            and string[3].lower() in ["v", "h"]
        ):
...
while True:
    ...
    if action == None:
        ...
        continue
    if action not in current_state.get_applicable_actions(
